...
/**
 * TODO: I don't see where I'm actually making use of these except
 *       to validate my results. I ended up manually padding.
 *       Can possibly remove.
 */


SECTION_CODE int pkcs7_padding_pad_buffer( uint8_t *buffer,  size_t data_length, size_t buffer_size, uint8_t modulus )
{
...
SECTION_CODE int pkcs7_padding_valid( uint8_t *buffer, size_t data_length, size_t buffer_size, uint8_t modulus )
{
...
SECTION_CODE size_t pkcs7_padding_data_length( uint8_t * buffer, size_t buffer_size, uint8_t modulus )
{
