// Credit: https://github.com/robertdavidgraham/whats-dec/blob/master/crypto-sha256.c
 ...
 */
#include "utility_hashing_sha256.h"
 ...
 */
SECTION_CODE static unsigned read_word(const unsigned char *p, size_t offset) {
 ...
 * See `read_word()` above
 */
SECTION_CODE static void write_word(unsigned char *p, unsigned x) {
 ...
 */
SECTION_CODE static inline unsigned ROTR(unsigned x, unsigned n) {
    return ((x >> n) | (x << (32 - n)));
}
 ...
 * σ₁(x) = ROTR¹⁷(x) ⊕ ROTR¹⁹(x) ⊕ SHR¹⁰(x)
 */
SECTION_CODE static inline unsigned Ch(unsigned x, unsigned y, unsigned z) {
    return (z ^ (x & (y ^ z)));
}
SECTION_CODE static inline unsigned Maj(unsigned x, unsigned y, unsigned z) {
    return (((x | y) & z) | (x & y));
}
SECTION_CODE static inline unsigned Sigma0(unsigned x) {
    return (ROTR(x, 2) ^ ROTR(x, 13) ^ ROTR(x, 22));
}
SECTION_CODE static inline unsigned Sigma1(unsigned x) {
    return (ROTR(x, 6) ^ ROTR(x, 11) ^ ROTR(x, 25));
}
SECTION_CODE static inline unsigned Gamma0(unsigned x) {
    return (ROTR(x, 7) ^ ROTR(x, 18) ^ (x >> 3));
}
SECTION_CODE static inline unsigned Gamma1(unsigned x) {
 ...
 */
SECTION_CODE int SHA256_Init(SHA256_CTX *ctx) {
    unsigned *H = ctx->h;
 ...
 */
SECTION_CODE void SHA256_Transform(SHA256_CTX *ctx, const unsigned char *buf) {
...
SECTION_CODE int SHA256_Update(SHA256_CTX *ctx, const void *vsrc, size_t length) {
...
SECTION_CODE int SHA256_Final(unsigned char *digest, SHA256_CTX *ctx) {
    unsigned int i;
...
SECTION_CODE unsigned char *SHA256(const unsigned char *message, size_t length,
